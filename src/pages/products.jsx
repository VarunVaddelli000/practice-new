    ...
    return (
        <div>
            ...
            <div className='flex gap-1 justify-center m-2'>
                <input
                    type='text' onChange={(event) => setSearchString(event.target.value)}
                    className='w-fit p-1 bg-gray-300 rounded-md border-blue-400'
                    placeholder='Search...' />
                <button className='bg-blue-600 p-1 rounded-md text-white'>Submit</button>
            </div>
            {filtredProducts.map((product, index) => (
                <div key={index} className="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6 flex items-start mb-2">
                    {/* Product Image */}
                    <div className="w-1/4">
                        <img
                            src={product.imgSrc}
