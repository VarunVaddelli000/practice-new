        ...
        <div>
            <div className="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6 flex  items-start mb-2">
                <div className="flex flex-col  space-x-2 mt-1">
                    <div className="bg-green-600 text-white text-sm font-semibold px-2 py-1 rounded w-fit mb-3">
                        {filteredProducts.ratings}
                    </div>
                    <div >
                        {/* Safely check if ProductDescription exists and has at least one item */}
                        <div className="border-slate-300 border-b-4">
                            <p className="font-semibold text-xl">{filteredProducts.reviews && filteredProducts.reviews.length > 0 ? filteredProducts.reviews[0].user : 'No reviews'}</p>
                            <p className="mb-2">{filteredProducts.reviews && filteredProducts.reviews.length > 0 ? filteredProducts.reviews[0].comment : 'No reviews'}</p>
                        </div>
                        <div className="mt-2">
                            <p className="font-semibold text-xl">{filteredProducts.reviews && filteredProducts.reviews.length > 0 ? filteredProducts.reviews[1].user : 'No reviews'}</p>
                            <p className="mb-2">{filteredProducts.reviews && filteredProducts.reviews.length > 0 ? filteredProducts.reviews[1].comment : 'No reviews'}</p>
                        </div>
                    </div>
                    <Outlet />
                </div>
